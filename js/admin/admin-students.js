...
async function addStudent() {
  ...
  if (!name || !matric || !department || !level) {
    err.textContent = "Fill in name, matric number, department, and level.";
    return;
  }
  err.textContent = "";
  ...
  if (error) {
    err.textContent =
      "Couldn't add student: " + (error.message || "unknown error");
    console.error(error);
    return;
  }
  ...
  $("#level").value = "";
  await loadStudents();
  toast(`Student added — login: ${matric}, password: their surname`);
}
